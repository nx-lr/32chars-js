...
function encode(
    ...
    let punct = '_$-,;:!?.@*/&#%^+<=>|~()[]{}\'"`\\'
    let alnum = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'
    ...
    function encodeCharKey(char) {
        ...
        if (/\d/.test(char)) {
            key = [...(+char).toString(2).padStart(3, 0)].map(x =>
                +x ? '$' : '_'
            ).join``
        }
...
.flat().join``'
    ...
    let bigRegExp = RegExp(
        _.entries(regExps).map(([key, {source}]) => `(?<${key}>${source})`)
            .join`|`,
        'giu'
    )
    ...
    let metadata = (() => {
        ...
        if (tokenGroups.Digit)
            tokenGroups.Digit = tokenGroups.Digit.filter(
                ([num]) => num >= 10
            ).map(([substr]) => [
                substr,
                keyGen.next().value,
